...
function(config) {
    var DiscussionModel = Backbone.Model.extend({
        ...
        // note that a GET on this URL will give us a single discussion, rather than an array that we normally expect
        // ie 1:1 with StratFile
        urlRoot: config.serverBaseUrl + "/stratfiles/%s/discussions",
        defaults: {
        },
        ...
        initialize:function(model, options) {
            ...
            console.debug('Initing discussion for stratfile: ' + model.stratFileId);
            if ("id" in model) {
        ...
        parse: function(json) {
            this.url = sprintf(this.urlRoot, this.get('stratFileId')) + "/" + json.data.discussion.id;
            return json.data.discussion;
        }
    }, {
